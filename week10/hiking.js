...
// function difficultySearch() {

//     let difficultyQuery = document.querySelector('#difficulty_search').value;
//     let filteredHikes = hikes.filter(function(hike){
//         return hike.difficulty === parseInt(difficultyQuery);
//     }
//     )
...
const difficultyButton = difficultyInput.nextElementSibling;

difficultyButton.addEventListener("click", () => {
    const value = difficultyInput.value.trim();

    const cards = document.querySelectorAll(".hike-card");

    cards.forEach(card => {
        const bootCount = card.querySelectorAll(".icon-boot").length;

        if (value === "" || Number(value) === bootCount) {
            card.style.display = "";
        } else {
...
document.getElementById("search").addEventListener("keypress", handleEnter);
